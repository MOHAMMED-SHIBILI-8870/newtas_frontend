    ...
    <div className="min-h-screen bg-slate-50 p-8 font-sans">
      <div className="mb-8 flex items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm">{filteredUsers.length} total registered users</p>
        </div>

        <button
          onClick={() => setIsModalOpen(true)}
          className="inline-flex items-center gap-2 rounded-xl bg-indigo-600 px-4 py-2 font-semibold text-white shadow-lg shadow-indigo-100 transition hover:bg-indigo-700"
        >
      ...
      <div className="mb-6 flex flex-wrap items-center gap-4 rounded-2xl border border-gray-100 bg-white p-4 shadow-sm">
        <div className="relative min-w-[260px] flex-1">
          ...
          <input
            ...
            className="w-full rounded-xl border border-slate-200 bg-slate-50 py-2 pl-10 pr-4 outline-none transition focus:ring-2 focus:ring-indigo-500/20"
            value={searchTerm}
        ...
        <select
          className="rounded-xl border border-slate-200 bg-slate-50 px-4 py-2 text-sm font-medium text-slate-600 outline-none"
          value={roleFilter}
      ...
      <div className="overflow-hidden rounded-3xl border border-gray-100 bg-white shadow-sm">
        {loading ? (
          <div className="flex items-center justify-center gap-2 p-20 text-indigo-600">
                ...
                return (
                  <tr
                    ...
                    className={`transition-colors ${
                      isCurrentUser ? 'border-l-4 border-indigo-500 bg-indigo-50' : 'hover:bg-slate-50/50'
                    }`}
                  >
                    ...
                    <td className="px-6 py-4">
                      <div className="flex w-fit items-center gap-2 rounded-lg border border-slate-100 bg-slate-50 px-2 py-1">
                        ...
                        <select
                          ...
                          className={`bg-transparent text-[11px] font-bold outline-none ${
                            isCurrentUser ? 'cursor-not-allowed text-slate-400' : 'cursor-pointer text-slate-600'
                          }`}
                        >
                          <option value="user">User</option>
                          <option value="admin">Admin</option>
                    ...
                    <td className="px-6 py-4">
                      <span
                        className={`rounded-lg px-2 py-1 text-[10px] font-bold uppercase tracking-wider ${
                          !item.is_blocked ? 'bg-emerald-50 text-emerald-600' : 'bg-red-50 text-red-500'
                        }`}
                      >
        ...
        {!loading && filteredUsers.length === 0 && (
          <div className="p-12 text-center font-medium text-slate-400">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 p-4 backdrop-blur-sm">
          <div className="w-full max-w-md overflow-hidden rounded-3xl border border-slate-100 bg-white shadow-2xl">
            ...
            <form onSubmit={handleCreateUser} className="space-y-4 p-6">
              <div>
                <label className="ml-1 text-xs font-bold uppercase text-slate-500">Full Name</label>
                <input
                  required
                  type="text"
                  ...
                  className="mt-1 w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-2 outline-none transition focus:ring-2 focus:ring-indigo-500/20"
                />
              </div>

              <div>
                <label className="ml-1 text-xs font-bold uppercase text-slate-500">Email Address</label>
                <input
                  required
                  type="email"
              ...
              <div>
                <label className="ml-1 text-xs font-bold uppercase text-slate-500">Password</label>
                <input
                  required
                  type="password"
                  placeholder="••••••••"
              ...
              <div>
                <label className="ml-1 text-xs font-bold uppercase text-slate-500">Assign Role</label>
                <select
                  value={newUser.role}
              ...
              <div className="flex gap-3 pt-4">
                <button
                  ...
                  className="flex-1 rounded-xl px-4 py-2 font-semibold text-slate-600 hover:bg-slate-50"
                >
                  Cancel
                </button>
                ...
                <button
                  ...
                  className="flex-1 inline-flex items-center justify-center gap-2 rounded-xl bg-indigo-600 px-4 py-2 font-semibold text-white shadow-lg shadow-indigo-100 disabled:opacity-70 hover:bg-indigo-700"
                >
