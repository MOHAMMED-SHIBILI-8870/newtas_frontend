...
const navigation = [
  {
    section: 'Driver Tasks',
    items: [
      { label: 'Dashboard', to: '/driver/dashboard', icon: LayoutDashboard },
      { label: 'Assigned Trips', to: '/driver/trips', icon: MapPinned },
      { label: 'Schedule', to: '/driver/schedule', icon: Files },
    ],
  },
  {
    section: 'Resources',
    items: [
      { label: 'My Vehicle', to: '/driver/vehicle', icon: Truck },
      { label: 'Live Tracking', to: '/driver/tracking', icon: Route },
...
export default function DriverSidebar({ collapsed, mobileOpen, onClose, onToggleCollapse, onLogout }) {
  ...
  const initials = useMemo(() => {
    const source = user?.full_name || user?.email || 'D'
    return source
      .trim()
      .split(/\s+/)
      .slice(0, 2)
          ...
          <Link to="/driver/dashboard" className="flex items-center gap-3">
            ...
            {!collapsed ? (
              <div>
                ...
                <p className="mt-1 text-sm font-semibold text-white/85">Driver Console</p>
              </div>
            ) : null}
          </Link>
          ...
          <div className="flex items-center gap-3 rounded-[24px] border border-white/10 bg-white/5 p-3">
            ...
            {!collapsed ? (
              <div className="min-w-0 flex-1">
                <p className="truncate text-sm font-semibold text-white">{user?.full_name || user?.email || 'Driver'}</p>
                <p className="truncate text-xs text-white/55">Driver Partner</p>
              </div>
            ) : null}
          </div>
        </div>
